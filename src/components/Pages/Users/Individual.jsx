...
export default function () {
    ...
    const url3 = "http://164.90.174.113:9090/Api/Admin/All/VehicleOwners/Role/individual";
    useEffect(() => {
        setLoading(true)
        fetch(url3, options)
            .then(respnse => respnse.json())
    ...
    const [cargoData1, setcargoData1] = useState([])
    const [cargoData, setCargoData] = useState([])
    const url4 = "http://164.90.174.113:9090/Api/Admin/All/CargoOwners";
    useEffect(() => {
        setLoading(true)
        fetch(url4, options)
            .then(respnse => respnse.json())
            .then(data => {
                setcargoData1(data.cargoOwners)
                setCargoData(data.cargoOwners.length)
                    ...
                    <div className={styles.vehicle}>
                        <Link style={{ textDecoration: 'none' }} to="/cargo">
                            <div className={styles.innerContents}>
                                <h4>cargo</h4>
                                <div>
                                    <p><FaUserAlt size="2rem"></FaUserAlt><b>{cargoData}</b></p>
                                </div>
                            </div>
                        </Link>
                    </div>
