...
export default function () {
    ...
    const url = "http://198.199.67.201:9090/Api/Vehicle/Alerts/AlertType/ACCIDENT";
    useEffect(() => {
        setLoading(true)
        fetch(url, options)
            .then(respnse => respnse.json())
            .then(data => {
                setDataSource(data.inactiveAlerts)
                setTotalPage(data.inactiveAlerts.length)
                setLoading(false)
            })
    }, [])

    const [dataSource2, setDataSource2] = useState([])
    const url2 = "http://198.199.67.201:9090/Api/Vehicle/Alerts/AlertType/OFFROAD";
    useEffect(() => {
        setLoading(true)
        fetch(url2, options)
            .then(respnse => respnse.json())
            .then(data => {
                setDataSource2(data.inactiveAlerts)
                setLoading(false)
            })
    }, [])

    const [dataSource3, setDataSource3] = useState([])
    const url3 = "http://198.199.67.201:9090/Api/Vehicle/Alerts/AlertType/DRIVER";
    useEffect(() => {
        setLoading(true)
        fetch(url3, options)
            .then(respnse => respnse.json())
            .then(data => {
                setDataSource3(data.inactiveAlerts)
                setLoading(false)
            })
    }, [])
